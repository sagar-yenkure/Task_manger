...
router.get('/Getnotes', fetchuser, async (req, res) => {
    //we gets the id of user by jwt token in fetch user middleware.
    try {
        ...
        res.status(500).send({ error: "some problem in getting all note." })
...
router.post('/Addnotes', fetchuser, [
    ...
    try {
        ...
        const note = new Notes({
            title, description, due_date,tag, user: req.user.id //here we are saving note with user is extracted by jwt
        })
...
router.put('/Upadtenote/:id', fetchuser, async (req, res) => { //here :id is the notes id ,not of user id
    ...
    const { title, description, tag ,due_date } = req.body;//destructring the elements
    //create a newNote object
    const newNote = {};
    if (title) { newNote.title = title }
    if (description) { newNote.description = description }
    if (tag) { newNote.tag = tag }
    if(due_date){newNote.due_date=due_date}
